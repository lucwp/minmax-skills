...
2. Require profile status `configured`, or an explicitly accepted `draft` with material gaps surfaced. If the profile is missing, `template`, stale, or materially incomplete, stop proposal generation and return control to the root orchestrator so it can run the internal `minmax-proposal-inquiry` skill. Do not run a parallel business-onboarding flow inside this worker.
...
6. If the user supplies a reusable business change while requesting a proposal, return control to the root orchestrator, run Inquiry first, then generate the proposal from the updated worker.
...
This worker reads reusable business configuration but does not own its mutation. The internal `minmax-proposal-inquiry` skill is responsible for changing offers, positioning, pricing policy, proof, brand assets, voice, and proposal defaults.
...
8. **Render standalone HTML.** Use `internal-skills/minmax-enterprise-proposal/scripts/render_proposal.py --profile internal-skills/minmax-enterprise-proposal/references/business-profile.json --proposal proposal.json --output proposal.html`.
9. **Run preflight.** Execute `internal-skills/minmax-enterprise-proposal/scripts/preflight_proposal.py proposal.html proposal.json` and fix every FAIL.

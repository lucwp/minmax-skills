...
description: Give ChatGPT durable, portable memory across conversations without a dedicated memory backend. Persist compact, human-readable Markdown through any compatible file or cloud-storage connector, using provider-native text when it offers better read/write or concurrency behavior. Use when the user asks to remember, recall, forget, correct, update, supersede, migrate, or continue prior work; when a durable preference, decision, constraint, project state, relationship, or open loop should persist; or when prior user/project context would materially improve correctness. Avoid ordinary one-off tasks that do not benefit from persistent memory.
---
...
5. Prefer one targeted storage connector read or write on the fast path. Avoid multi-file cascades unless the task genuinely needs deep recall or conflict repair.
...
12. Never blind-overwrite a memory document from stale context. Use provider version checks when available; otherwise re-read immediately before a full-document update and abort or rebase on concurrent change.
13. Use Markdown as the canonical logical serialization for memory. Keep headings, lists, metadata, links, and durable entries Markdown-compatible regardless of the provider's physical file type.
14. Choose the physical representation by connector capability: prefer a real `.md` file when direct text read/update and safe concurrency are efficient; otherwise use a provider-native text document that preserves the same Markdown-compatible content. Do not convert formats merely for cosmetic consistency.
...
Use an available connected cloud-drive/storage connector that satisfies the capability contract below. Resolve the root folder named `MinMax Agent Memory` before reads or writes. For backward compatibility, also check the legacy root name `AI Memory`; if exactly one trusted legacy root exists, treat it as the same store and rename it to `MinMax Agent Memory` when the connector supports safe rename and a write operation is already warranted. Never create a second store merely because the canonical name is absent.

If no trusted store exists and a writable compatible connector is available, initialize the canonical root only when the user requested memory setup or a durable write requires storage. If no writable compatible connector exists, do not attempt initialization; continue the user task without persistent mutation and state the limitation briefly.

## Connector capability contract

Select storage by capabilities, not vendor name. A provider is **read-compatible** when it can locate/list folders/files and read text content. It is **write-compatible** when it can additionally create folders/files and update existing memory content. Search is preferred but optional because scoped listing plus direct reads can provide a degraded fallback. `DELETE`, safe rename/move, and revision/version checks are optional capabilities that strengthen forgetting, migration, and concurrency safety.

Provider selection order:

1. If the user explicitly selects a provider that already contains a trusted active store, use it.
2. Otherwise reuse the provider/store already bound to the current workflow.
3. Otherwise reuse the provider containing exactly one trusted canonical `MinMax Agent Memory` root.
4. Otherwise reuse the provider containing exactly one trusted legacy `AI Memory` root.
5. Otherwise honor an explicit provider choice for initialization if it is write-compatible.
6. Otherwise choose one write-compatible connected provider deterministically and initialize only when persistent setup or a durable write requires it.
7. If the user explicitly asks to switch providers while another active store exists, route to **MIGRATE**; never start writing to a second store as an implicit switch.

Treat duplicate active roots on the same provider or across providers as split-brain. Do not merge, alternate writes, or choose solely by folder name. For recall, use the already-bound or explicitly selected trusted store when unambiguous; for a write, stop if canonical ownership remains ambiguous.

Do not require provider-specific APIs, MIME types, or native document formats in the memory model. Map logical operations `LIST`, `READ`, optional `SEARCH`, `CREATE`, `UPDATE`, optional `DELETE`, optional `RENAME/MOVE`, and optional `VERSION_CHECK` onto connector actions.

### Serialization and physical-format negotiation

Treat Markdown as the memory system's canonical serialization, not as a mandatory provider MIME type. Logical memory documents are `MEMORY.md`, `Preferences.md`, `Decisions.md`, `Context.md`, project `Memory.md`, entity memories, and selected Evidence documents.

Choose storage representation in this order:

1. Prefer a physical `.md`/plain-text file when the connector supports efficient direct text reads, content updates, scoped discovery, and adequate concurrency protection.
2. Prefer a provider-native text document when it offers materially safer or cheaper partial edits/version checks than raw-file replacement. Keep its body Markdown-compatible and map it to the same logical `.md` path.
...
5. Do not migrate an existing healthy store between native and `.md` representation solely because this policy changes. Migrate format only when the user explicitly requests it or measured connector behavior shows a material reliability/performance benefit.
...
When a native provider document represents a logical Markdown file, the visible provider title may omit `.md` if the provider's native format makes extensions unnatural. The store marker and layout define logical identity.

## Store trust and identity

A folder name alone is not proof of canonical memory. Treat a root as trusted when at least one is true: the user explicitly selected it; the current workflow was already bound to it; or it contains the expected layout plus a valid store marker in `Core/MEMORY`. When provider metadata exposes ownership/sharing, do not silently adopt an arbitrary shared/public same-named root over a private/owned or previously bound store.
...
- **MIGRATE** — the user explicitly changes the canonical storage provider/root while an active store already exists.
- **BOTH** — recall is needed before acting and the interaction also changes durable state.
- **NOOP** — after inspection, persistent memory is not materially useful. Exit without storage connector calls where possible.
...
1. Resolve logical `MinMax Agent Memory/Core/MEMORY.md` first only when scope is unclear; a provider-native document titled `MEMORY` may represent that logical file.
...
5. Search deeper only when the target is missing, ambiguous, stale, or the user asks for exact historical rationale. Keep memory search scoped to `MinMax Agent Memory` or its known subfolders whenever the connector permits; do not rely on unconstrained whole-provider search.
6. If a semantic/paraphrased query returns no useful connector results, do not conclude that no memory exists. Resolve the likely scope/project/entity by name or alias, open its memory document, and inspect it semantically before declaring a miss.
7. When degraded retrieval relies on paginated folder listing, follow continuation tokens/pages within the narrow target folder until the target is found or that folder is exhausted. Do not page through the entire provider as a substitute for scoped discovery.

Target: 1-2 storage connector operations for ordinary recall.
...
4. If the value has a dedicated Evidence artifact and `DELETE` is available, delete it when the forget request covers that evidence. If delete is unavailable, remove it from all active retrieval surfaces and overwrite/redact only when the connector safely supports that operation.
5. If provider revision history, retention, backups, or shared copies may preserve old bytes, distinguish **logical forgetting from the active memory system** from guaranteed physical erasure. Do not promise irreversible deletion beyond connector capabilities.
...
Treat an explicit provider switch as **MIGRATE**, never as implicit initialization of a second truth store.
...
3. Create a new canonical root on the target with a new Store ID, copy the memory tree, and verify readable content plus expected document/folder counts or equivalent provider evidence.
...
A read-only active store remains valid for RECALL. If WRITE, REPAIR, or FORGET is requested and the active store is read-only, do not create a second writable store automatically. Mutate only after the user explicitly authorizes migration/copy to a write-compatible provider or the existing provider becomes writable.
...
Optimize for `useful remembered context / incremental tokens + connector calls + maintenance`.
...
- Keep memory discovery inside `MinMax Agent Memory` whenever possible; unrelated provider documents are retrieval noise, not memory candidates.
...
- average storage connector operations for fast recall/write;
...
- provider migration cutover and read-only behavior;
- concurrent-write conflict handling with and without version checks;
- pagination and partial-failure recovery;
- Markdown serialization fidelity across physical provider formats;
- physical `.md` vs provider-native read/write operation count and concurrency behavior;
...
- storage structure, provider selection, capability negotiation, or initialization -> `references/storage-layout.md`
- root trust, split-brain, concurrency, forgetting, migration, degraded/write safety -> `references/provider-safety.md`
